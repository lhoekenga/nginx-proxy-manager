...
const setupDefaultSettings = () => {
    return Promise.all([
        // Check and insert 'default-site' setting
        settingModel
            .query()
            .select('id')
            ...
            .then((row) => {
                if (!row) {
                    return settingModel
                        ...
                        .then(() => logger.info('Added default-site setting'));
                }
            }),

        // Check and insert 'oidc-config' setting
        settingModel
            .query()
            .select('id')
            ...
            .then((row) => {
                if (!row) {
                    return settingModel
                        ...
                        .then(() => logger.info('Added oidc-config setting'));
                }
            }),
    ])
    .then(() => {
